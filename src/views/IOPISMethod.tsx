...
function IOPISMethod({
    ...
    const startMethod = async () => {
      try {
        ...
        if (res.status == 200) {
          ...
          SetAlternatives(
            ParseSolutions(
              [activeProblemInfo.ideal].concat([activeProblemInfo.nadir]),
              activeProblemInfo!
            )
          );
  ...
  return (
    <Container>
      <h3 className="mb-3">{"IOPIS/NSGAIII"}</h3>
      <>
